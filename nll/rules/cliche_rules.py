...
def find_matches_for_echoing_sentences(text: str) -> list[TextMatch]:
...
def find_matches_for_question_chains(text: str) -> list[TextMatch]:
...
def find_matches_for_repeated_sentence_openers(text: str) -> list[TextMatch]:
...
class Cliche014(CodeRule, identifier="CLH014"):
    def __call__(self, document: Document) -> Violations:
        return build_violations_for_matches(
            self,
            document,
            find_matches_for_echoing_sentences(document.prose),
        )
...
class Cliche021(CodeRule, identifier="CLH021"):
    def __call__(self, document: Document) -> Violations:
        return build_violations_for_matches(
            self,
            document,
            find_matches_for_question_chains(document.prose),
        )
...
class Cliche022(CodeRule, identifier="CLH022"):
    def __call__(self, document: Document) -> Violations:
        return build_violations_for_matches(
            self,
            document,
            find_matches_for_repeated_sentence_openers(document.prose),
        )
