...
def test_other_non_ascii_skips_characters_with_dedicated_rules(
    ...
    rule = next(
        rule
        for rule in rulebook.rules_definitions.iter_rules()
        if rule.identifier == "CHR000"
    )

    violations = list(rule(Document(prose="café — · –", path="notes.md")))
...
def test_other_non_ascii_reports_unassigned_unicode_characters(
    ...
    violations = list(rule(Document(prose="a\U000e0080b", path="notes.md")))
...
def test_sentence_rule_counts_sentences_ending_in_punctuation(
    ...
    rule = next(
        rule
        for rule in rulebook.rules_definitions.iter_rules()
        if rule.identifier == "LEN001"
    )

    violations = list(rule(Document(prose="One. Two! Three? Four.", path="notes.md")))

    assert len(violations) == 1
    assert violations[0].path == "notes.md"
...
def test_sentence_rule_ignores_text_at_or_below_the_limit(
    ...
    assert list(rule(Document(prose="One. Two. Three.", path="notes.md"))) == []
